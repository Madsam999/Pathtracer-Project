...
je vais mettre des vidéos dans le dossier \texttt{Results/Videos} pour montrer la différence de performance. La sphère jaune bouge en orbite 
autour de la boule verte. Dans quelques images débruitées, l'interface graphique dit que j'utilise 5 rayons par pixels, cependant
cette valeur est la valeur par défaut et l'algorithme de SVGF n'utilise qu'un seul rayon par pixel (j'ignore la variable rayons par pixels
durant le processus du SVGF).
\begin{figure}[H]
    \centering
